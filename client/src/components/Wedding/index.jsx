...
function Wedding() {
    ...
    const filteredMeetings = wedding.filter(
      (wedding) =>
        wedding.title.toLowerCase().includes(categoryFilter.toLowerCase()) &&
        (priceFilter === "" ||wedding.price <= parseInt(priceFilter)) &&
        (guestsFilter === "" ||
          (guestsFilter === "1-50" &&wedding.guests <= 50) ||
          (guestsFilter === "50-100" &&wedding.guests > 50 && wedding.guests <= 100) ||
          (guestsFilter === "100+" &&wedding.guests > 100)) &&
        (locationFilter === "" ||wedding.handle.toLowerCase().includes(locationFilter.toLowerCase()))
    );
